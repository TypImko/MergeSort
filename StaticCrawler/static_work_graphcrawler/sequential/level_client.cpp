...
vector<vector<string>> bfs(CURL* curl, const string& start, int depth) {
  ...
  auto Frontier = [&] (int d, string& s, int startP, int end){
    try {
      threadMutex.lock();
      if (debug)
        std::cout<<"Trying to expand"<<s<<"\n";
      vector<string> neighbor = get_neighbors(fetch_neighbors(curl, s));
      for (int i = startP; i < end; i++) {
        if (debug)
          std::cout<<"neighbor "<<neighbor[i]<<"\n";
        if (!visited.count(neighbor[i])) {
          visited.insert(neighbor[i]);
          levels[d+1].push_back(neighbor[i]);
        }
      }
      threadMutex.unlock();
          } catch (const ParseException& e) {
  ...
  for (int d = 0;  d < depth; d++) {
    ...
    for (string& s : levels[d]) {
      ...
      for(int i = 0; i < maxThreads; i++){
        ...
        std::thread cThread(Frontier, d, s, startPoint, end);
